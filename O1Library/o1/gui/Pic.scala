...
import smcl.pictures.{Bitmap as SMCLBitmap, Picture as SMCLPicture, PictureElement as SMCLPictureElement}
import smcl.settings.*
import o1.gui.compat.*
import o1.gui.Anchor.*
import o1.gui.PicHistory.op.{AdjustViewport,Create,Miscellaneous,Transform}
...
object Pic extends ShapeAPI:
  ...
  def apply(pathOrURL: String, anchor: Anchor): Pic =
    ...
    val newHistory = PicHistory(Create(method = "Pic".p, simpleDescription = pathOrURL))
  ...
  private[gui] final def apply(smclContent: Option[SMCLPictureElement], o1Anchor: Anchor, history: PicHistory): Pic =
    val wrappedContent = smclContent.map( _.toPicture) getOrElse SMCLPicture()
  ...
  def generate(width: Int, height: Int, makeColor: (Int, Int) => Color): Pic =
    ...
    val anchor = Center
    val newHistory = PicHistory(Create(method = "Pic.generate".p, simpleDescription = "generated pic"))
  ...
  def fromColors(width: Int, height: Int, colors: Seq[Color]): Pic =
    if colors == null then throw IllegalArgumentException("The sequence of pixel colors cannot be null")
    val smclContent = smcl.pictures.Bitmap(width, height, colors.map( _.smclColor ))
    val anchor = Center
    val newHistory = PicHistory(Create(method = "Pic.fromColors".p, simpleDescription = "composed pic"))
    Pic(smclContent, anchor, newHistory)
...
final class Pic private[gui](private[gui] val smclPic: SMCLPicture, itsAnchor: Anchor,
  ...
  /** Forces rasterization of any vector graphics in this `Pic`’s internal representation
    ...
    * @see [[isBitmap]] */
  def freeze: Pic =
    Pic(this.toSMCLBitmap, this.anchor, Miscellaneous("freeze".p) :: this.historyDetails)
  ...
  private[gui] lazy val boundary: Bounds =
    (this.smclPic.viewport.map( _.boundary ) getOrElse this.smclPic.boundary).toO1Bounds
  ...
  def anchorAt(anchor: Anchor): Pic =
    Pic(this.smclPic, anchor, Miscellaneous("anchorAt".p) :: this.historyDetails)
  ...
  inline private[gui] def setViewportToContentBoundary: Pic = setViewportToContentBoundaryOf(this)

  inline private[gui] def setViewportToContentBoundaryOf(picture: Pic): Pic =
    if picture.smclPic.boundary.isUndefined then this else setViewport(picture.smclPic.boundary.toO1Bounds)
  ...
  inline private[gui] def setViewport(viewport: Viewport): Pic =
    this.setViewportInternal(viewport, AdjustViewport("setViewport".p) :: this.historyDetails)
  ...
  inline private[gui] def removeViewport: Pic =
    removeViewportInternal(viewport, AdjustViewport("removeViewport".p) :: this.historyDetails)
  ...
  lazy val toImage: Option[BufferedImage] =
    import smcl.infrastructure.jvmawt.SMCLBitmapWrapper
    toSMCLBitmap.toAWTImage     // Unless this pic is already in bitmap form, this will be less efficient as the pic will first be rasterized.

  /** This `Pic` converted to a [[javax.swing.Icon Swing `Icon`]]. */
  lazy val toIcon: Option[Icon] =
    import smcl.infrastructure.jvmawt.SMCLBitmapWrapper
    toSMCLBitmap.toSwingIcon
  ...
  def save(path: String): Boolean = toSMCLBitmap.saveAsPngTo(path)
  ...
  /** Returns the color of the pixel at the given coordinates within the image.
    ...
    * If you have a vector graphic and need to call this method many times, consider
    * [[freeze freezing]] the `Pic` first. */
  def apply(pixelX: Int, pixelY: Int): Color = this.pixelColor(pixelX, pixelY)
  ...
  /** Returns the color of the pixel at the given coordinates within the image.
    ...
    * If you have a vector graphic and need to call this method many times, consider
    * [[freeze freezing]] the `Pic` first. */
  def apply(pixelPos: Pos): Color = this.pixelColor(pixelPos)
  ...
  def transformColors(transformer: Color => Color): Pic =
    ...
    val smclContent = toSMCLBitmap.transformColorToColor(callPasser)
    Pic(smclContent, this.anchor, Transform("transformColors".p) :: this.historyDetails)
  ...
  def transformXY(newColorAt: (Int, Int) => Color): Pic =
    ...
    Pic(smclContent, this.anchor, Transform("transformXY".p) :: this.historyDetails)
  ...
  def crop(boundary: Bounds): Pic =
    ...
    val ulCorner = Pos(left, top).toSMCLPos
    val smclContent = this.smclPic.crop(ulCorner, width atLeast 0, height atLeast 0).toPicture
    val smclContentWithViewport =
      if this.smclPic.hasViewport then smclContent.setViewportToContentBoundary(name = None) else smclContent
    Pic(smclContentWithViewport, this.anchor, Transform("crop".p) :: this.historyDetails)

  /** Returns a partial `Pic` of a rectangular area within this `Pic`.
    * @param topLeft      the cropping frame’s top-left corner within this `Pic`; this point must not be outside the `Pic`
    * @param bottomRight  the cropping frame’s bottom-right corner within this `Pic`; this point must not be out side the `Pic`
    * @return a new `Pic` (a bitmap) that contains a part of the original */
  ...
  def crop(topLeft: Pos, width: Double, height: Double): Pic =
    ...
    val smclContent = this.smclPic.crop(ulCorner, nWidth atLeast 0, nHeight atLeast 0).toPicture
    val smclContentWithViewport =
      if this.smclPic.hasViewport then smclContent.setViewportToContentBoundary(name = None) else smclContent
    Pic(smclContentWithViewport, this.anchor, Transform("crop".p) :: this.historyDetails)
  ...
  def scaleBy(factor: Double): Pic =
    ...
    Pic(smclContent, this.anchor, Transform("scaleBy".p) :: this.historyDetails)
  ...
  def scaleTo(targetWidth: Double, targetHeight: Double): Pic =
    ...
    Pic(smclContent, this.anchor, Transform("scaleTo".p) :: this.historyDetails)
  ...
  /** Creates a scaled version of this `Pic` that has the dimensions of the given `Pic`.
    ...
    * @return a new `Pic` that is a scaled version of this one; it will usually be
    *         a bitmap, but some simple vector-based `Pic`s may retain their vector form */
  def scaleTo(sizingPic: Pic): Pic = this.scaleTo(sizingPic.width, sizingPic.height)
  ...
  def flipHorizontal: Pic =
    ...
    Pic(smclContentWithViewport, this.anchor, Transform("flipHorizontal".p) :: this.historyDetails)
  ...
  def flipVertical: Pic =
    ...
    val smclContentWithViewport = this.smclPic.viewport.map(smclContent.setViewport) getOrElse smclContent
    Pic(smclContentWithViewport, this.anchor, Transform("flipVertical".p) :: this.historyDetails)
  ...
  def flipDiagonal: Pic =
    ...
    val smclContentWithViewport = this.smclPic.viewport.map(smclContent.setViewport) getOrElse smclContent
    Pic(smclContentWithViewport, this.anchor, Transform("flipDiagonal".p) :: this.historyDetails)
  ...
  def clockwise(degrees: Double = 90.0): Pic =
    val smclContent = if degrees == 90.0 then this.smclPic.rotateBy90DegsCW(centerOfRotation)
                      else                    this.smclPic.rotateBy(-degrees, centerOfRotation)
    Pic(Some(smclContent), this.anchor, Transform("clockwise".p) :: this.historyDetails)
  ...
  def counterclockwise(degrees: Double = 90.0): Pic =
    val smclContent = if degrees == 90.0 then this.smclPic.rotateBy90DegsCCW(centerOfRotation)
                      else                    this.smclPic.rotateBy(degrees, centerOfRotation)
    Pic(Some(smclContent), this.anchor, Transform("counterclockwise".p) :: this.historyDetails)
  ...
  def shiftRight(offset: Double): Pic =
    ...
    if normalizedOffset == 0 then
      ...
      Pic(this.shiftedContentInViewport(smclContent), this.anchor, Transform("shiftRight".p) :: this.historyDetails)
  ...
  def shiftLeft(offset: Double): Pic =
    ...
    if normalizedOffset == 0 then
      ...
      Pic(this.shiftedContentInViewport(smclContent), this.anchor, Transform("shiftLeft".p) :: this.historyDetails)
  ...
  def shiftDown(offset: Double): Pic =
    ...
    if normalizedOffset == 0 then
      ...
      Pic(this.shiftedContentInViewport(smclContent), this.anchor, Transform("shiftDown".p) :: this.historyDetails)
  ...
  def shiftUp(offset: Double): Pic =
    ...
    if normalizedOffset == 0 then
      ...
      Pic(this.shiftedContentInViewport(smclContent), this.anchor, Transform("shiftUp".p) :: this.historyDetails)
  ...
  private def placePicPos(picPos: (Pic, Pos)): Pic = this.place(picPos(0), picPos(1))
  ...
  infix def leftOf(rightPic: Pic, retainAnchor: Boolean = false): Pic =
  ...
  infix def rightOf(leftPic: Pic, retainAnchor: Boolean = false): Pic =
  ...
  infix def below(abovePic: Pic, retainAnchor: Boolean = false): Pic =
  ...
  infix def above(lowerPic: Pic, retainAnchor: Boolean = false): Pic =
  ...
  inline private def historyForCombinedPicture: PicHistory =
    val creation = Create(method = "<combining method>".p, simpleDescription = "combined pic")
