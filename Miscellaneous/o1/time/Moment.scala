...
/** Each instance of the class `Moment` represents a “moment” on a time scale,
  ...
  *
  * @param time  an integer indicating which moment the object should represent (2023, for instance) */
class Moment(private val time: Int):
  ...
  /** Determines the length of time between this moment and another given moment.
    ...
    * 2023, returns 23. The distance is always either positive or zero. */
  ...
  def isIn(anInterval: Interval) = anInterval.contains(this)
