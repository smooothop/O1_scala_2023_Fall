...
object ExampleLeague:
  ...
  val EVE = Club("Everton",              "EVE", "Goodison Park")
  val CAR = Club("Cardiff",              "CAR", "Cardiff City Stadium")
  val WH  = Club("West Ham United",      "WH",  "Upton Park")
  val CP  = Club("Crystal Palace",       "CP",  "Selhurst Park")
  val MU  = Club("Manchester United",    "MU",  "Old Trafford")
  val STO = Club("Stoke City",           "STO", "Britannia Stadium")
  val HUL = Club("Hull City",            "HUL", "KC Stadium")
  val NEW = Club("Newcastle United",     "NEW", "St. James' Park")
  val LIV = Club("Liverpool",            "LIV", "Anfield")
  val AV  = Club("Aston Villa",          "AV",  "Villa Park")
  val SOT = Club("Southampton",          "SOT", "St. Mary's Stadium")
  val TOT = Club("Tottenham Hotspur",    "TOT", "White Hart Lane")
  val MC  = Club("Manchester City",      "MC",  "Etihad Stadium")
  val WBA = Club("West Bromwich Albion", "WBA", "The Hawthorns")
  val SWA = Club("Swansea City",         "SWA", "Liberty Stadium")
  val NOR = Club("Norwich City",         "NOR", "Carrow Road")
  ...
  val Players: Map[Club, Vector[Player]] = Seq(
    ...
    KH  -> Vector("Elmo")
  ).map(playersForNames).toMap

  private def playersForNames(club: Club, names: Vector[String]) =
    club -> names.map( name => Player(name, club) )
